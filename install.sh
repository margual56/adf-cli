...
# Set target to ~/bin if ADF_CLI_RUNTIME_VERSION environment variable is set.
if [ -n "$ADF_CLI_RUNTIME_VERSION" ]; then
...
if [ ! -w "$TARGET" ]; then
    ...
    echo "  curl -fsSL https://raw.githubusercontent.com/margual56/adf-cli/v${VERSION}/install.sh | sudo sh"
    exit 1
fi

# Make sure we don't overwrite an existing installation.
if [ -f "$TARGET/adf-cli" ]; then
    echo "Target path $TARGET/adf-cli already exists."
    echo "If you have an existing ADF CLI installation, please first remove it using"
    echo "  sudo rm '$TARGET/adf-cli'"
    exit 1
fi
